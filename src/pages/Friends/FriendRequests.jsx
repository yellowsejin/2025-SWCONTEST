...
import { doc, getDoc, setDoc } from "firebase/firestore";  // ⬅️ setDoc 추가
...
export default function FriendRequests() {
  ...
  // ✅ 동기화 함수
  const syncFriends = async (fromUid, toUid) => {
    await Promise.all([
    ...
    console.log("✅ users/{uid}/friends 동기화 완료:", fromUid, toUid);
  };

  const onAccept = async (req) => {
    try {
      setBusyId(req.id);
      // 1. friendRequests status 변경
      await acceptRequest(req.id);

      // 2. users/{uid}/friends 동기화
      const fromUid = req.from;
      const toUid = req.to;
      await syncFriends(fromUid, toUid);
    } finally {
      setBusyId(null);
    }
  };
